    ...
    simdb_tester_(std::make_shared<sparta_simdb::DatabaseTester>())
{
    ...
    controller_.reset(new OlympiaSim::ExampleController(this));
...
void OlympiaSim::registerStatCalculationFcns_()
{
...
auto OlympiaSim::getCPUFactory_() -> core_example::CPUFactory*{
...
void OlympiaSim::bindTree_()
{
    ...
    sparta::SpartaHandler cb = sparta::SpartaHandler::from_member<
        OlympiaSim, &OlympiaSim::postRandomNumber_>(
            this, "OlympiaSim::postRandomNumber_");
    ...
    toggle_notif_trigger_.reset(new sparta::trigger::ExpressionTimeTrigger(
        "ToggleNotif",
        CREATE_SPARTA_HANDLER(OlympiaSim, postToToggleTrigger_),
        "1 ns",
        getRoot()));

    lazy_table_create_trigger_.reset(new sparta::trigger::ExpressionTrigger(
        "DelayedTableCreate",
        CREATE_SPARTA_HANDLER(OlympiaSim, addToStatsSchema_),
    ...
    for (uint32_t core_idx = 0; core_idx < num_cores_; ++core_idx) {
        core_warmup_listeners_.emplace_back(
            new sparta::trigger::ExpressionTrigger(
                ...
                CREATE_SPARTA_HANDLER(OlympiaSim, onLegacyWarmupNotification_),
...
void OlympiaSim::onLegacyWarmupNotification_()
{
...
void OlympiaSim::postRandomNumber_()
{
    ...
    // the OlympiaSim's constructor earlier on.
...
void OlympiaSim::postToToggleTrigger_()
{
...
void OlympiaSim::addToStatsSchema_()
{
    if (auto db_root = getDatabaseRoot()) {
        if (auto db_namespace = db_root->getNamespace("Stats")) {
            ...
            lazy_table_create_trigger_.reset(new sparta::trigger::ExpressionTrigger(
                "DelayedTableCreate",
                CREATE_SPARTA_HANDLER(OlympiaSim, addToLazySchemaTable_),
...
void OlympiaSim::addToLazySchemaTable_()
{
...
void OlympiaSim::onTriggered_(const std::string & msg)
{
    std::cout << "     [trigger] " << msg << std::endl;
}

void OlympiaSim::validateTreeNodeExtensions_()
{
...
OlympiaSim::ExampleController::ExampleController(
...
void OlympiaSim::ExampleController::pause_(const sparta::app::Simulation * sim)
{
    std::cout << "  [control] Controller PAUSE method has been called for simulation '"
              << sim->getSimName() << "'" << std::endl;
}

void OlympiaSim::ExampleController::resume_(const sparta::app::Simulation * sim)
{
    std::cout << "  [control] Controller RESUME method has been called for simulation '"
              << sim->getSimName() << "'" << std::endl;
}

void OlympiaSim::ExampleController::terminate_(const sparta::app::Simulation * sim)
{
    std::cout << "  [control] Controller TERMINATE method has been called for simulation '"
              << sim->getSimName() << "'" << std::endl;
}

void OlympiaSim::ExampleController::customEatCallback_()
{
    std::cout << "  [control] Controller CUSTOM method has been called ('eat')" << std::endl;
}

void OlympiaSim::ExampleController::customSleepCallback_()
{
